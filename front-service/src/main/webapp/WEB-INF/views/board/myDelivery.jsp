...
<div class="chat" id="app" >
	<div class="chat-title">
		<h1 style="font-size: 15px"> My Deliveries </h1>
		<h2> </h2>
		<figure class="avatar">
	...
	<div class="messages">
		<div class="messages-content" >
			<li class="list-group-item list-group-item-action" style="margin: 5px 0px 5px 0px; background-color:gold; border-radius: 5px;"  v-for="item in chatrooms" v-bind:key="item.roomId" v-on:click="enterRoom(item.roomId, item.rentalReqName, item.rentalReqDepartment, item.bookName, item.bookId)">
				<div class="row">
					<div class="col-3">
	...
	let vm = new Vue({
		...
		methods: {
			findAllRoom: function() {
				axios.get('http://poly-library.com/chat-service/rooms/'+userUuid+'/deliveries').then(response => { this.chatrooms = response.data; });
			},
			createRoom: function() {
				alert("createRoom!!!")
				if("" === this.bookId) {
					...
					axios({
						method: 'post',
						url: '/chat-service/rooms/'+userUuid+'/deliveries',
						headers: {
			...
			enterRoom: function(roomId, rentalReqName, rentalReqDepartment, bookName, bookId) {
				let name = sessionStorage.getItem("nickname");
				let userid = sessionStorage.getItem("userUuid");
				sessionStorage.setItem('opponentName', rentalReqName);
				sessionStorage.setItem('opponentDepartment', rentalReqDepartment);
